...
def validate_q_length(q: List[str] = Query(default=[])):
    if len(q) > settings.limit.batch_items:
        _raise_422(
            "q",
            f"Query parameter 'q' cannot have more than {settings.limit.batch_items} items.",
        )
    return q
...
def validate_limit(limit: int = Query(default=10)):
    if limit > settings.limit.sql_limit:
        _raise_422(
            "limit",
            f"Query parameter 'limit' cannot be greater than {settings.limit.sql_limit}.",
        )
    return limit
...
async def validate_token_nullable(
    request: Request,
    token_header: HTTPAuthorizationCredentials = Depends(_security),
...
async def validate_token(
    ...
    if token_header is None and token is None:
        raise HTTPException(
            status_code=401,
            detail="You are accessing a protected route. Please sign in or provide an API key.",
        )
    return await validate_token_nullable(request, token_header, token)


async def validate_one_time_code(
    request: Request,
    ...
    if state is None:
        ...
        raise HTTPException(
            status_code=400,
            detail="Code not found. Please check if the link is correct.",
        )
    # check one_time_code
    ...
    if one_time_code.revoked:
        raise HTTPException(
            status_code=400,
            detail="Code revoked. This usually means some have tried to authenticate using this link.",
        )
    if one_time_code.expires < int(time.time()):
        raise HTTPException(
            status_code=400, detail="Code expired. Please try logging in again."
        )

    # check if the client ip and user agent is the same
    if (
        one_time_code.user_agent != client_info["user_agent"]
        or one_time_code.request_ip != client_info["request_ip"]
    ):
        logger.warning(
            f"User-Agent or IP mismatch: "
...
def get_httpx_client(request: Request):
    if hasattr(request.app.state, "httpx_client") and isinstance(
        request.app.state.httpx_client, AsyncClient
    ):
