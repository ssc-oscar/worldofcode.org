...
@api.get("/object/{type_}", response_model=WocResponse[dict], dependencies=[Depends(validate_q_length)])
def show_contents(request: Request, type_: ObjectName, q: List[str] = Query(...), raw: bool = False):
    """
...
@api.get("/object/{type_}/{key}", response_model=Union[WocResponse[Union[str,list]], str])
def show_content(request: Request, type_: ObjectName, key: str, raw: bool = False):
    ...
    try:
        if raw is True:
            return Response(content=_show_content(woc, type_, key, raw=True), media_type="text/plain")
...
@api.get("/map/{map_}", dependencies=[Depends(validate_q_length)], response_model=WocResponse[Dict[str, list]])
def get_values(request: Request, map_: str, q: List[str] = Query(...)):
    """
    Get the values of a batch of keys from a map. The map can be any of the following:
...
def get_value(request: Request, map_: str, key: str):
    """
    Get the value of a key from a map. The map can be any of the following:
    c2fbb obb2cf bb2cf a2f A2f P2a b2P b2f a2P b2fa b2tac c2p c2cc p2a A2a a2A c2dat 
    a2c P2c P2p c2P p2P A2c A2P P2A a2p A2b A2fb P2b P2fb c2b p2c
    Check https://github.com/woc-hack/tutorial for the complete list.
